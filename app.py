...
def get_db():
    try:
        ...
        db.close()
#updating to use session from the dependency instead of cs connection 
...
def get_emailids(db=Depends(get_db)):
    ...
    """
    sql = text(f"SELECT * FROM ps_testtable")
    try:
...
def update_emailids(emplid: str, email_id: str, db=Depends(get_db)):
    """
    Updates the email ID for a given employee ID in the database.
    """
    sql = text(f"UPDATE ps_testtable SET EMAIL = '{email_id}' WHERE ID = '{emplid}'")
    try:
        with db.begin():
...
def add_emailids(emplid: str, email_id: str, db=Depends(get_db)):
    """
    Adds a new employee ID and email ID to the database.
    """
    sql = text(f"INSERT INTO ps_testtable (ID, EMAIL) VALUES ('{emplid}', '{email_id}')")
    try:
        with db.begin():
            test = db.execute(sql)  # Use db.execute instead of cs.execute
            db.commit()  # Commit the changes made in this transaction
